...
public:
    ...
    void moveFromCustomMsg(livox_ros_driver2::msg::CustomMsg &Msg, pcl::PointCloud<PointXYZIRT> & cloud)
    {
        ...
        // Rotate raw sensor-frame points into the gravity-aligned body frame
        // using the same extrinsic rotation that imuConverter applies to the
        // IMU. Required when the lidar+IMU unit is mounted upside-down or
        // tilted so that LIO-SAM's Z-up assumption holds in the body frame.
        Eigen::Matrix3f rot = extRot.cast<float>();
        for(uint i=0;i<Msg.point_num-1;i++)
        {
            Eigen::Vector3f p_raw(Msg.points[i].x, Msg.points[i].y, Msg.points[i].z);
            Eigen::Vector3f p_body = rot * p_raw;
            point.x=p_body.x();
            point.y=p_body.y();
            point.z=p_body.z();
