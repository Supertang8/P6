...
def generate_launch_description():
    ...
    lidar_topic_arg = DeclareLaunchArgument(
        'lidar_topic', default_value='/livox/lidar_192_168_1_122')
    aggregated_topic_arg = DeclareLaunchArgument(
        'aggregated_topic', default_value='/drone/aggregated_pointcloud')
    trigger_service_arg = DeclareLaunchArgument(
        'trigger_service', default_value='/drone/trigger_accumulation')
    output_frame_id_arg = DeclareLaunchArgument(
        'output_frame_id', default_value='drone_lidar')
    ...
    livox_launch_file = PythonLaunchDescriptionSource([
        livox_package_path, '/launch/msg_MID360_launch.py'
    ])
